"""Rigid + Shear Transformations between GMMs

Transform a GMM by a global affine transform (scaling + rotation + shear) and a translation. Also use this transform model to register one GMM onto another. Note that this assumes isotropic scaling.
"""
...
def transform_gmm_rotangles(
    ...
    if n_dim == 2:
        return transform_gmm_rotangles2(
            means, covariances, scale, alpha, k_xy, k_xz, translation
        )
    else:
...
def _create_optimization_function(
    ...
    def loss_l2(
        ...
        # unpack parameters
        scale, alpha, beta, gamma, k_xy, k_xz, k_yx, k_yz, k_zx, k_zy, trans = (
