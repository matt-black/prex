...
This is a copy of `tree.py` from the `jaxkd` library (https://github.com/dodgebc/jaxkd/blob/main/jaxkd/tree.py)
"""
...
def build_tree(
    points: Array, optimize: bool = True, cuda: bool = False
) -> tree_type:
    ...
    if points.ndim != 2:
        raise ValueError(
            f"Points must have shape (N, d). Got shape {points.shape}."
        )
    if points.shape[-1] >= 128:
        raise ValueError(
            f"Maximum dimension 127, got {points.shape[-1]} dimensions."
        )
    ...
    if not cuda:
        ...
        points, indices, split_dims = (
            jaxkd_cuda.build_tree(  # pyright: ignore[reportPossiblyUnboundVariable]
                points
            )
        )
...
def query_neighbors(
    ...
    if k > len(tree.points):
        raise ValueError(
            f"Queried {k} neighbors but tree contains only {len(tree.points)} points."
        )

    query_shaped = jnp.atleast_2d(query)

    if not cuda:
        neighbors, distances = jax.vmap(
            lambda q: _single_query_neighbors(tree, q, k=k)
        )(query_shaped)
    else:
        if not global_has_cuda:
            raise ImportError("jaxkd-cuda extension is not installed")
        if tree.split_dims is None:
            raise ValueError(
                "jaxkd-cuda extension requires optimize=True, i.e. split_dims=None"
            )
        neighbors, distances = (
            jaxkd_cuda.query_neighbors(  # pyright: ignore[reportPossiblyUnboundVariable]
...
def count_neighbors(
    ...
    if (
        ...
        raise ValueError(
            f"Invalid shape for query {query.shape} or radius {r.shape}."
        )

    query_shaped = jnp.atleast_2d(query)
    r_shaped = jnp.atleast_2d(r)
    r_shaped = jnp.broadcast_to(
        r_shaped, (query_shaped.shape[0], r_shaped.shape[-1])
    )
    ...
    if not cuda:
        ...
        counts = jaxkd_cuda.count_neighbors(  # pyright: ignore[reportPossiblyUnboundVariable]
...
def _check_tree(tree: tree_type) -> None:
    ...
    if len(points) != len(indices):
        raise ValueError(
            f"Invalid tree, {len(points)} points and {len(indices)} indices."
        )
    if split_dims is not None and len(split_dims) != len(points):
        raise ValueError(
            f"Invalid tree, {len(points)} points and {len(split_dims)} split dims."
        )
    if jax.config.jax_enable_x64 is False and len(points) > 2**32 - 1:  # type: ignore
        raise ValueError(
            f"len(points)={len(points)} exceeds maximum value of int32, try int64"
        )


def _single_query_neighbors(
    tree: tree_type, query: Array, *, k: int
) -> tuple[Array, Array]:
    ...
    def update_func(node, state, _):
        ...
        square_distance = jnp.sum(
            jnp.square(query - points[indices[node]]), axis=-1
        )
    ...
    distances, neighbors = lax.sort(
        (distances, neighbors), dimension=0, num_keys=2
    )
    return neighbors, distances


def _single_count_neighbors(
    tree: tree_type, query: Array, *, r: float | Array
) -> Array:
    ...
    count = _traverse_tree(
        tree, query, update_func, count, jnp.max(jnp.square(r))
    )
    return count
...
def _build_tree(points: Array, optimize: bool = True) -> tree_type:
    ...
    def step(carry, level):
        ...
        if optimize:
            dim_max = jax.ops.segment_max(
                points[indices], nodes, num_segments=n_points
            )
            dim_min = jax.ops.segment_min(
                points[indices], nodes, num_segments=n_points
            )
            new_split_dims = jnp.asarray(
                jnp.argmax(dim_max - dim_min, axis=-1), dtype=jnp.int8
            )
            ...
            points_along_dim = jnp.squeeze(
                jnp.take_along_axis(
                    points[indices], split_dims[nodes][:, jnp.newaxis], axis=-1
                ),
                axis=-1,
            )
        else:
            split_dim = jnp.asarray(
                jnp.mod(level, points.shape[-1]), dtype=jnp.int8
            )
        ...
        branch_start = (
            ...
            + n_left_siblings
            * ((1 << height) - 1)  # left sibling internal descendants
            + jnp.minimum(
                n_left_siblings * (1 << height),
                n_points - ((1 << (n_levels - 1)) - 1),
            )
...
def _traverse_tree(
    ...
    def step(carry):
        ...
        level = (
            jnp.frexp(current + 1)[1] - 1
        )  # robust way top compute floor(log2(current + 1))
        split_dim = (
            jnp.mod(level, points.shape[-1])
            if split_dims is None
            else split_dims[current]
        )
