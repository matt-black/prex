...
#     # scale_f, alpha_f, beta_f, gamma_f, trans_f = unpack_params_3d(par_f)
...
#     )
#     scale = jnp.array(2.0)
#     mu2 = transform_means_rotangles2(mu, scale, alpha, trans)
#     rescaling = 1 / self_energy_gmm_spherical(mu2, wgt, 1.0, n_dim).item()
#     max_iter = 100
#     par_init = pack_params_2d(
#         jnp.array(1.0), jnp.array(0.0), jnp.zeros((n_dim,))
#     )
#     par_f, (_, _, num_iter) = opt.spherical(
#         mu2,
#         wgt,
#         mu,
#         wgt,
#         opt.AlignmentMethod.RIGID,
#         opt.DistanceFunction.L2,
#         (opt.Regularization.NONE, 0.0),
#         par_init,
#         var_ref=1.0,
#         var_mov=1.0,
#         grad_tol=-jnp.inf,
...
#     )
#     assert num_iter < max_iter
#     scale_f, alpha_f, trans_f = unpack_params_2d(par_f)
#     assert jnp.isclose(scale, scale_f, atol=1e-3)
#     assert jnp.isclose(alpha, alpha_f, atol=1e-3)
#     assert jnp.allclose(trans, trans_f, atol=1e-2)
